...
def get_signal(df_15m, df_5m):
    try:
        ...
        logger.info(f"Signal check | trend_bull={trend_bull} | SMC_score={score} | AI={ai_prob:.1f} | Final={final_confidence} | XGBoost={'ON' if USE_XGBOOST else 'OFF'}")
        ...
        if trend_bear and final_confidence >= 55:
            sl = entry + atr
            # FIXED: Correct SHORT TP calculation (was inverted before)
