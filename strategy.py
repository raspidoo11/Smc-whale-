...
def bullish_bos(df):
    ...
    if USE_XGBOOST:
        # Strict BOS + 2-candle confirmation (reduces false breakouts)
        return (
            df["close"].iloc[-1] > swing_high
            and df["close"].iloc[-2] > swing_high * 0.998
        )

    # Relaxed BOS for SMC mode + 2-candle confirmation
    close1_ok = (
        df["close"].iloc[-1] >= swing_high * 0.9995
        or df["high"].iloc[-1] > swing_high
    )
    close2_ok = (
        df["close"].iloc[-2] >= swing_high * 0.9995
        or df["high"].iloc[-2] > swing_high
    )
    return close1_ok and close2_ok
...
def bearish_bos(df):
    ...
    if USE_XGBOOST:
        # Strict BOS + 2-candle confirmation
        return (
            df["close"].iloc[-1] < swing_low
            and df["close"].iloc[-2] < swing_low * 1.002
        )

    # Relaxed BOS for SMC mode + 2-candle confirmation
    close1_ok = (
        df["close"].iloc[-1] <= swing_low * 1.0005
        or df["low"].iloc[-1] < swing_low
    )
    close2_ok = (
        df["close"].iloc[-2] <= swing_low * 1.0005
        or df["low"].iloc[-2] < swing_low
    )
    return close1_ok and close2_ok
