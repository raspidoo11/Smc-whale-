...
def train_model_incremental():
    ...
    logger.info(f"💾 Saving feature names to: data/models/feature_names.pkl")
    joblib.dump(X.columns.tolist(), "data/models/feature_names.pkl")
    logger.info(f"✅ Feature names saved!")
...
def get_xgboost_probability(trade_features):
    """Get AI probability for a trade setup - WITH DEBUG LOGGING"""
    
    logger.info(f"🔍 [AI DEBUG] Checking for model at: {MODEL_PATH}")
    
    if not Path(MODEL_PATH).exists():
        logger.warning(f"❌ [AI DEBUG] Model file not found at {MODEL_PATH}")
        return 50.0

    logger.info(f"✅ [AI DEBUG] Model file exists!")
    
    try:
        logger.info(f"🔍 [AI DEBUG] Loading model...")
        model = joblib.load(MODEL_PATH)
        logger.info(f"✅ [AI DEBUG] Model loaded!")
        
        logger.info(f"🔍 [AI DEBUG] Loading feature names from data/models/feature_names.pkl...")
        feature_names = joblib.load("data/models/feature_names.pkl")
        logger.info(f"✅ [AI DEBUG] Features loaded! Got {len(feature_names)} features")
        logger.info(f"🔍 [AI DEBUG] Feature list: {feature_names}")
        
        logger.info(f"🔍 [AI DEBUG] Creating DataFrame with {len(trade_features)} input features...")
        X = pd.DataFrame([trade_features])
        logger.info(f"✅ [AI DEBUG] DataFrame shape: {X.shape}")
        logger.info(f"🔍 [AI DEBUG] Input columns: {sorted(list(X.columns))}")
        
        logger.info(f"🔍 [AI DEBUG] Selecting columns from feature_names...")
        X = X[feature_names]
        logger.info(f"✅ [AI DEBUG] Selected columns!")
        
        logger.info(f"🔍 [AI DEBUG] Making prediction...")
        prob = model.predict_proba(X)[0][1] * 100
        logger.info(f"✅ [AI DEBUG] AI Probability: {prob:.1f}%")
        
        return round(prob, 1)
        
    except FileNotFoundError as e:
        logger.error(f"❌ [AI DEBUG] FILE NOT FOUND: {e}")
        return 50.0
    except KeyError as e:
        logger.error(f"❌ [AI DEBUG] MISSING FEATURE: {e}")
        logger.error(f"❌ [AI DEBUG] Expected features: {feature_names}")
        logger.error(f"❌ [AI DEBUG] Got features: {list(trade_features.keys())}")
        return 50.0
    except Exception as e:
        logger.error(f"❌ [AI DEBUG] XGBoost failed: {type(e).__name__}: {e}")
        import traceback
